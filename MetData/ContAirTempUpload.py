######################################################
# GET DAILY AIR TEMP DATA FROM GEOMET AND UPLOAD TO NG #
######################################################
import requests
import os
...
def getStationIds():
    ...
    if exists:
        with open(path + '\\TTresultListAirTemp.txt') as fp:
            line = fp.readline()
            while line:
...
def getGeoMetData(station, parameter, EndTime, token):
    ...
    for x in stnData:
        ...
        if date >= formattedEndTime:
            airTemp = x['properties']['MEAN_TEMPERATURE']
            TSId = getTsId(station, parameter, token)
            uploadTempToNg(TSId, airTemp, date)

def uploadTempToNg(TSId, airTemp, date):
        url = "https://wsc.aquaticinformatics.net/AQUARIUS/Acquisition/v2/timeseries/" + str(TSId) + "/append"
        Udata = '{"UniqueId": "' + str(TSId) + '", "Points": [{"Time": "' + str(date) + '", "Value": ' + str(airTemp) + ', "Type": "Point", "GradeCode": -1, "Qualifiers": ""}]}'
        headers = {'Content-Type': 'application/json', 'Accept': 'application/json'}
        try:
