########################################################################################################
# DOWNLOAD MINIMUM AIR TEMPERATURE DATA FROM 3.10 THEN UPLOAD TO NG (ONE TIME PROCESS FOR HISTORICAL DATA) #
########################################################################################################
import csv
import StringIO
import requests
...
userName = "USERNAME"
password = "PASSWORD"
server_pub = "https://wscdev.aquaticinformatics.net/AQUARIUS/publish/v2/"
server_pro = "https://wscdev.aquaticinformatics.net/AQUARIUS/Provisioning/v1/"
server_acq = "https://wscdev.aquaticinformatics.net/AQUARIUS/Acquisition/v2/"
log = open("LOG PATH", "a+")
...
def downloadAirMinData(stations, tokenTT):
    for station in stations:
        ...
        endDate = datetime.datetime(2020, 4, 27, 1, 0, 0, 0)
        # TODO
        Timeseries = "Air Temp.DAILY MINIMUM TEMPERATURE@"+str(station)
        ...
        if result == 200:
            ...
            file = open("C:\\Users\\YanX\\Desktop\\scripts\\\MetData\\NoMetAirMin.txt", "a+")
            file.write(str(station) + "\n")
            print ("No Air Min Temp data for " + str(station))
...
def outputResultFile(station, stationName, lat, long, resultText):
    ...
    file = open("C:\\Users\\YanX\\Desktop\\scripts\\\MetData\\TTresultListAirMin.txt", "a+")
    ...
    outputFile = open(outputPath + "\\" + station + "_airMin.txt", "w+")
    for m in resultText:
        for n in m:
            outputFile.write(n)
    outputFile.close()
...
def getResultStations():
    ...
    with open("C:\\Users\\YanX\\Desktop\\scripts\\\MetData\\TTresultListAirMin.txt") as fp:
        line = fp.readline()
        while line:
            id = line.split(",")
...
def createNGTimeSerie(stn, stationUniqueId, userName, password, server_pro):
    ...
    TSdata = '{"GapTolerance": "PT1441M", "LocationUniqueId": "' + str(stationUniqueId) + '", "Label": "Minimum Air Temperature", "Parameter": "TA_min",' \
...
def getTimeSerieUniqueId(stn, tokenNG, server_pub):
    ...
    try:
        ...
        print ("Unable to get Unique Id for station: " + str(stn))
        log.write("Unable to get Unique Id for station: " + str(stn) + "\n")

    for x in TSdesc:
        param = x['Parameter']
        if param == "Min Air Temp":
...
def uploadMetData(stn, tsUniqueId, username, password, server_acq):
    ...
    with open(airPath + str(stn) + '_airMin.txt') as fp:
        line = fp.readline()
        line = fp.readline()
        line = fp.readline()
        line = fp.readline()
...
def main():
    ...
    tokenTT = getLoginTT()
    downloadAirMinData(stations, tokenTT)
