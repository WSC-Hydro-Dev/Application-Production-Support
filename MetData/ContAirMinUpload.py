#######################################################
# GET DAILY AIR MIN DATA FROM GEOMET AND UPLOAD TO NG #
#######################################################
import requests
import os
...
def getStationIds():
    ...
    if exists:
        with open(path + '\\TTresultListAirMin.txt') as fp:
            line = fp.readline()
            while line:
...
def getGeoMetData(station, parameter, EndTime, token):
    ...
    for x in stnData:
        ...
        if date >= formattedEndTime:
            airMinTemp = x['properties']['MIN_TEMPERATURE']
            uploadTempToNg(TSId, airMinTemp, date)

def uploadTempToNg(TSId, airMinTemp, date):
        url = "https://wsc.aquaticinformatics.net/AQUARIUS/Acquisition/v2/timeseries/" + str(TSId) + "/append"
        Udata = '{"UniqueId": "' + str(TSId) + '", "Points": [{"Time": "' + str(date) + '", "Value": ' + str(airMinTemp) + ', "Type": "Point", "GradeCode": -1, "Qualifiers": ""}]}'
        headers = {'Content-Type': 'application/json', 'Accept': 'application/json'}
        try:
