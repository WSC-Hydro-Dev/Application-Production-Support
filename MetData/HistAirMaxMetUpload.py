########################################################################################################
# DOWNLOAD MAXIMUM AIR TEMPERATURE DATA FROM 3.10 THEN UPLOAD TO NG (ONE TIME PROCESS FOR HISTORICAL DATA) #
########################################################################################################
import csv
import StringIO
import requests
...
def downloadAirMaxData(stations, tokenTT):
    for station in stations:
        ...
        # TODO
        Timeseries = "Air Temp.DAILY MAXIMUM TEMPERATURE@"+str(station)
        ...
        if result == 200:
            ...
            file = open("SOME PATH", "a+")
            file.write(str(station) + "\n")
            print ("No Air Max Temp data for " + str(station))
...
def outputResultFile(station, stationName, lat, long, resultText):
    ...
    outputFile = open(outputPath + "\\" + station + "_airMax.txt", "w+")
    for m in resultText:
        for n in m:
            outputFile.write(n)
    outputFile.close()
...
def createNGTimeSerie(stn, stationUniqueId, userName, password, server_pro):
    ...
    TSdata = '{"GapTolerance": "PT1441M", "LocationUniqueId": "' + str(stationUniqueId) + '", "Label": "Maximum Air Temperature", "Parameter": "TA_max",' \
...
def uploadMetData(stn, tsUniqueId, username, password, server_acq):
    ...
    data = []
    airPath = "PATH"
    with open(airPath + str(stn) + '_airMax.txt') as fp:
        line = fp.readline()
        line = fp.readline()
        line = fp.readline()
        line = fp.readline()
...
def main():
    ...
    tokenTT = getLoginTT()
    downloadAirMaxData(stations, tokenTT)
