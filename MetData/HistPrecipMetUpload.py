########################################################################################################
# DOWNLOAD TOTAL PRECIPITATION DATA FROM 3.10 THEN UPLOAD TO NG (ONE TIME PROCESS FOR HISTORICAL DATA) #
########################################################################################################
import csv
import StringIO
import requests
...
userName = "USERNAME"
password = "PASSWORD"
server_pub = "http://wscdev.aquaticinformatics.net/AQUARIUS/publish/v2/"
...
def downloadPrecipData(stations, tokenTT):
    for station in stations:
        ...
        Timeseries = "Precip Inc.DAILY TOTAL PRECIPITATION@"+str(station)
        ...
        if result == 200:
            ...
            file = open("C:\\Users\\YanX\\Desktop\\scripts\\\MetData\\NoMetPrecip.txt", "a+")
            file.write(str(station) + "\n")
            print "No Precip data for " + str(station)
...
def outputResultFile(station, stationName, lat, long, resultText):
    ...
    file = open("C:\\Users\\YanX\\Desktop\\scripts\\\MetData\\TTresultListPrecip.txt", "a+")
    ...
    outputFile = open(outputPath + "\\" + station + "_precip.txt", "w+")
    for m in resultText:
        for n in m:
            outputFile.write(n)
    outputFile.close()
...
def getResultStations():
    ...
    with open("C:\\Users\\YanX\\Desktop\\scripts\\\MetData\\TTresultListPrecip.txt") as fp:
        line = fp.readline()
        while line:
            id = line.split(",")
...
def uploadMetData(stn, tsUniqueId, username, password, server_acq):
    ...
    with open(airPath + str(stn) + '_precip.txt') as fp:
        line = fp.readline()
        line = fp.readline()
        line = fp.readline()
        line = fp.readline()
...
def main():
    ...
    tokenTT = getLoginTT()
    downloadPrecipData(stations, tokenTT)
