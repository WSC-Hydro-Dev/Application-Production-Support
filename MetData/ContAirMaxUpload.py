#######################################################
# GET DAILY AIR MAX DATA FROM GEOMET AND UPLOAD TO NG #
#######################################################
import requests
import os
...
def getStationIds():
    ...
    if exists:
        with open(path + '\\TTresultListAirMax.txt') as fp:
            line = fp.readline()
            while line:
...
def getGeoMetData(station, parameter, EndTime, token):
    ...
    for x in stnData:
        ...
        if date >= formattedEndTime:
            airMaxTemp = x['properties']['MAX_TEMPERATURE']
            uploadTempToNg(TSId, airMaxTemp, date)

def uploadTempToNg(TSId, airMaxTemp, date):
        url = "https://wsc.aquaticinformatics.net/AQUARIUS/Acquisition/v2/timeseries/" + str(TSId) + "/append"
        Udata = '{"UniqueId": "' + str(TSId) + '", "Points": [{"Time": "' + str(date) + '", "Value": ' + str(airMaxTemp) + ', "Type": "Point", "GradeCode": -1, "Qualifiers": ""}]}'
        headers = {'Content-Type': 'application/json', 'Accept': 'application/json'}
        try:
